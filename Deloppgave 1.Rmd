...
tec.err <- data %>% # Lager ett nytt objekt som skal inneholde estimatet av technical error
  ...
  kable(col.names = c("Standardavvik (ml/min)", # Lager tabellene og endrer overskriftene
                      "Gj.sn. testresultat (ml/min)", 
                      "Absolutt TE (ml/min)", 
                      "Relativ TE (%)"),
        digits = c(0, 0, 0, 1), # Endrer antall desimaler i tabellen
        caption = "Tabell 1: Estimat av technical error. Standardavvik representerer standardavviket av endringen fra pre til post-test.") %>% # Endrer overskriften
...
swcvo2.max <- data %>% # Lager ett nytt objekt som skal inneholde estimatet av smallest worthwhile change
  ...
  kable(col.names = c("Standardavvik (ml/min)", "SWC (ml/min)"), # Lager tabellen og endrer overskriftene
        digits = c(1,1), # Endrer antall desimaler i tabellen
        caption = "Tabell 2: Estimat av den minste betydningsfulle forandringen (SWC) og standardavvik.") %>% # Endrer overskriften
