...
raw.est.figur <- est %>%
  ...
  geom_point(position = position_dodge(width = 0.2)) +
  labs(x = "", y = "1RM (kg)",
       color = "Gruppe", title = "Figur 1") + # Nye akseetiketter
...
koef %>%
  data.frame() %>%
  mutate(coef = factor(coef, levels = c("(Intercept)",
                                        "timepointmid",
                                        "timepointpost",
                                        "groupRM10",
...
sum1 %>%
  mutate(koef = factor("koef", levels = c("(Intercept)",
                                        "timepointmid",
                                        "timepointpost",
                                        "groupRM10",
                                        "timepointmid:groupRM10",
...
raw.est.figur2 <- est2 %>% # Lager et nytt objekt med utgangspunkt i est2
  ...
  labs(x = "", y = "Isometrisk styrke (N)", 
       color = "Gruppe", title = "Figur 2") + # Setter nye navn på aksene og "legenden"
...
koeftabell2 <- sum2 %>%
  ...
  kable_styling() # Gjør tabellen penere
