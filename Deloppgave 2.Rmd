...
hyptable %>% # Bruker objektet hyptable til å lage tabellen
  kable(col.names = c("Variabel", "LOW (n = 10)", "HIGH (n = 10)"), # Endrer overskrifter
        caption = "Tabell 1: Forsøkspersonene i de to gruppene ved pre-test. Verdiene er oppgitt i gjennomsnitt og (standardavvik).") %>% # Endrer tittelen
...
df %>%
  ...
  kable(col.names = c("Gruppe", "Variabel", "n", "Gj.sn. endring (%)", "Standardavvik (%)"),
        caption = "Tabell 2: Gjennomsnittlige endringer i type II fiber fra pre- til post-test i de to gruppene") %>% #Lag tabell
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover")) # Endrer formateringen
...
ggqqplot(hyp, x = "T3T1_PERCENT_CHANGE_FAST_CSA", facet.by = "CLUSTER") + # Lager plottet med utgangspunkt i variablene
  labs(x = "Teoretiske kvantiler", y = "Dataenes kvantiler", caption = "QQ-plottet viser at dataene er tilnærmet normalfordelt.", cex.title=0.5, title = "Figur 1") +
...
stat.test %>% # Lager et nytt objekt som skal inneholde tabellen
  ...
  kable(col.names = c("Gruppe 1", "Gruppe 2", "N 1", "N 2", "P-verdi"), # Endrer overskriftene
        caption = "Tabell 2: Resultater t-test", # Endrer tittelen
...
cohentable <- cohen %>% # Lager et nytt objekt som skal inneholde tabellen
  mutate(.y. = factor(.y., levels = c("T3T1_PERCENT_CHANGE_FAST_CSA"), # Endrer navnet på variabelen
                           labels = c("Type II fiber change (%)")),
         magnitude = factor(magnitude, labels = c("Stor effekt"))) %>% # Endrer navnet på magnitude fra engelsk til norsk
  kable(col.names = c("Variabel", "Gruppe 1", "Gruppe 2", "Effektstørrelse", "N 1", "N 2", "Størrelse"), # Endrer overskriftene i tabellen
        digits = c(NA, NA, NA, 2, 0, 0, NA), # Endrer antall desimaler
        caption = "Tabell 3: Resultater Cohens D-formel") %>% # Endrer tittelen
  kable_styling(full_width = F, bootstrap_options = c("striped", "hover")) # Endrer formateringen
...
bxp + 
  ...
  labs(subtitle = get_test_label(stat.test, detailed = TRUE),
       caption = "Oppsummering av t-testen. Boxplottet viser endringene i type II fiber i de to gruppene fra pre- til post-test", title = "Figur 2") +
